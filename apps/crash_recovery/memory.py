...
class Memory:
    ...
    def recall_prior(self, query: str, entry_types: list[str] | None = None,
                     limit: int = 5, include_linked_runs: bool = False,
        ...
        for e in evidence:
            ...
            lines.append(f"[{e.get('entry_type')}]{stale_marker} {e.get('content', '')[:2000]}")
        return "\n".join(lines)
    ...
    def get_context(self, query: str, max_tokens: int = 1200) -> str:
        ...
        result = self.client.get_context(
            session_id=self.session_id, query=query,
            entry_types=["mental_model", "fact", "lesson", "rule", "feedback", "step_outcome"],
