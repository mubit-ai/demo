...
Run from the app folder: python -m tests.check_live
"""
import json
import subprocess
import sys
...
def main():
    ...
    if len(sys.argv) == 1:
        ...
        for phase in ("teach", "compare"):
            subprocess.run([sys.executable, "-m", "tests.check_live", phase, experiment], check=True, cwd=ROOT)
    ...
    try:
        ...
        model.close()
        path = ROOT / ".demo" / f"{experiment}-{phase}.json"
